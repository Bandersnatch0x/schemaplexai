...
If no arguments, check `.Codex/workflow-loop.local.md` for active loop state.

## State File

Location: `.Codex/workflow-loop.local.md`
...
2. Create state file at `.Codex/workflow-loop.local.md` with initial state
...
1. Read `.Codex/workflow-loop.local.md`
...
**Action**: Create/update `.Codex/changes/<feature-name>/proposal.md` using the template at `.Codex/workflow/templates/change-proposal.md`.
...
1. **Spawn 4 expert agents in parallel**, each reading `.Codex/changes/<feature-name>/proposal.md` + `context.md`:
...
4. **Write review report**: `.Codex/changes/<feature-name>/review-report.md`
...
**Action**: Create/update `.Codex/changes/<feature-name>/spec.md` using the template at `.Codex/workflow/templates/change-spec.md`.
...
- Read `AGENTS.md` for service map and patterns
...
**Action**: Create/update `.Codex/changes/<feature-name>/design.md` and `context.md`.
...
- Verify service ports and prefixes from AGENTS.md
...
**Action**: Create/update `.Codex/changes/<feature-name>/tasks.md` using the template at `.Codex/workflow/templates/change-tasks.md`.
...
3. **Visual Verification** (frontend changes):
   ...
   - Screenshots saved to `.Codex/outputs/screenshots/` for human review
   - Visual regression baseline: `npx playwright test --update-snapshots`

4. **Code Review**:
   - Spawn `code-reviewer` agent; require output to `.Codex/changes/<feature-name>/review-verdict.json` per its output contract
   - Spawn `security-reviewer` agent; require output to `.Codex/changes/<feature-name>/security-verdict.json` per its output contract
...
6. **Write delivery report**: `.Codex/changes/<feature-name>/delivery-report.md`
...
Process:
1. Run `.Codex/workflow/scripts/change-archive.sh <feature-name>`
...
7. **合并 Reflexion 评分报告**: 收集 `.Codex/changes/<feature>/judge-report-*.md`，合并为 `docs/archive/<feature>-<date>/reflexion-report.md`，包含：
   - 每个阶段的 Judge 评分
   - 总体加权平均分
   - 主要改进项和已解决项
...
6. 将评分报告写入 .Codex/changes/<feature>/judge-report-<phase>.md
```
...
每次 Judge 评分后，写入：
`.Codex/changes/<feature>/judge-report-<phase>.md`
...
1. Report final summary:
   ...
   - Coverage
   - Total iterations
2. Clean up: remove `.Codex/workflow-loop.local.md`
