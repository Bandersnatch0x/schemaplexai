---
name: "source-command-expert-testdoc"
description: "TestDocSentinel —— 测试覆盖 + 文档 drift 双轨；复用 code-reviewer + doc-gardener agents"
---

# source-command-expert-testdoc

Use this skill when the user asks to run the migrated source command `expert-testdoc`.
...
测试覆盖与文档 drift 哨兵。**强制复用** `.Codex/agents/code-reviewer.md`（测试质量）+ `.Codex/agents/doc-gardener.md`（文档 drift）双 agent。任务是把"幽灵 CI"修成"真 CI"，把 9 处文档 drift 清零。

## 强制复用

按需调用 Agent 工具：
- `subagent_type=code-reviewer` —— 测试质量评估、新增测试 PR 草案
- `subagent_type=doc-gardener` —— 文档 drift 检测、wiki/AGENTS.md/README 校准
...
- doc-gardener drift = 0（AGENTS.md / wiki / README）
- Knife4j 100% 覆盖维持
...
| AGENTS.md drift | 比对 `wiki/active-areas.md`、`wiki/gaps.md` 与 AGENTS.md 数字 |
...
```
- <date> [Δ] 评分 X.X → Y.Y；JaCoCo 模块覆盖 ?/16；前端 line ?%；AGENTS.md drift ?/9；e2e job 状态：[幽灵/进行中/绿]
```
...
> 「AGENTS.md 都写错模块数（12 vs 实际 16），还有哪些文档默认在骗人？」

## 红线

- **不动代码** —— 仅 PR 草案 + ADR
- **doc drift 必须列具体行号** —— "AGENTS.md 不准" 不够
