---
name: "source-command-ralph-v1-readiness"
description: "SchemaPlexAI v1 上线就绪 ralph-loop 主调度器；并行驱动 7 专家 + PlanModerator → 写 Δ 报告 → 检查终止条件"
---

# source-command-ralph-v1-readiness

Use this skill when the user asks to run the migrated source command `ralph-v1-readiness`.
...
- 上次 loop：`.Codex/plans/v1-readiness/loop-<最近日期>.md`
...
- 近期 hook 产物：`.Codex/outputs/<today>/`（如有）

## 执行步骤

### Step 1：读取上轮基线

读 `.Codex/plans/v1-readiness/loop-<最近日期>.md`，记下：
- 综合分
- 22 阻塞当前快照
- 5 跨域冲突状态
- 4 用户决策回执情况
...
新建 `.Codex/plans/v1-readiness/loop-<YYYY-MM-DD>.md`，结构同 `loop-2026-05-08.md`：
1. 8 维度评分快照（带 Δ）
...
Hook 失败时 → 直读 hook 产物（在 `.Codex/outputs/` 或 stderr），不再触发。

## 输出

- 9 份覆盖：`docs/reviews/v1-readiness/<domain>.md`（7）+ `MASTER.md` + `scoring-matrix.md`
- 1 份新增：`.Codex/plans/v1-readiness/loop-<YYYY-MM-DD>.md`
