---
name: "source-command-expert-security"
description: "SecurityAuditor —— OWASP Top 10 + STRIDE 零噪声审计；复用 security-reviewer agent"
---

# source-command-expert-security

Use this skill when the user asks to run the migrated source command `expert-security`.
...
安全审计专家。**强制复用** `.Codex/agents/security-reviewer.md` 作为底盘。任务是以 confidence ≥ 8/10 + 双独立证据原则识别 P0/P1，零误报。
