...
| # | 前置条件 | 验收标准 |
|---|---------|---------|
| 1 | SkillLoader 沙箱化解析 | Markdown 在独立 ExecutorService 线程池（非请求线程）解析，frontmatter name≤64/description≤500，禁止 HTML 标签，解析超时 5s |
    ...
    public ToolResult execute(ToolCall toolCall, ExecutionContext ctx) {
        ...
        try {
            SubAgentRequest request = SubAgentRequest.builder()
                ...
                .prompt(prompt)
                .role(role)
                // 不传 workspaceRoot — 子 Agent 默认获得独立 SandboxSession
...
public class AgentSessionPersistence {
    ...
    public void restoreSession(SfAgentExecution execution, SandboxSession sandbox) {
        ...
        if (!Files.exists(snapshotDir)) {
            log.warn("No snapshot found for execution {}, skipping restore", execution.getExecutionId());
            return;
        }
        try {
            copyDirectory(snapshotDir.resolve("workspace"), sandbox.workspaceRoot());
        } catch (IOException e) {
            log.error("Failed to restore snapshot for execution {}", execution.getExecutionId(), e);
            // 快照损坏不阻塞执行，降级为全新沙箱
        }
    }
}
...
| 恢复项 | 数据来源 | Token 预算 |
|--------|---------|-----------|
| 最近访问的文件 | `ConversationFileTracker`（Redis ZSET，记录最近 read 的文件路径 + 内容摘要，按 LRU 淘汰） | 50K, 最多 5 个 |
| Plan 文件 | `SubTaskPlan` JSON（从 `sf_agent_execution.metadata` 读取） | 不限 |
| 已调用的 Skill | `SkillRegistry` 按 execution 的 `skillName` 加载 | 5K/skill, 总计 25K |

**新增组件**: `ConversationFileTracker` — 在 `FileReadAdapter` 执行后记录文件路径到 Redis ZSET `sf:files:{conversationId}`，score 为时间戳，member 为文件路径。Compaction 时读取 top-N 文件内容恢复。
...
private boolean isSensitiveKey(String key) {
    ...
    // P0 验收标准：PASSWORD / SECRET / TOKEN / KEY
